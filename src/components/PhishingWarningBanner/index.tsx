...
const Container = styled(Flex)` 
  ...
  ${({ theme }) => theme.mediaQueries.md} {
    padding: 0px;
    background: linear-gradient(180deg, #3399ff 0%, #3399ff 100%);
  }
`
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningTextComponent = (
    <>
      <Text as="span" color="#ffff00" small bold textTransform="uppercase" >
        {t('Phishing warning: ')}
      </Text>
      {warningTextAsParts.map((text, i) => (
        <Text 
          ...
          color={text === 'https://pancakeswap.finance' ? '#00004d' :'#000000'  }
        >
          {text}
        </Text>
      ))}
      ...
      {isMobile || isMd ? (
        <>
          ...
          <IconButton onClick={hideBanner} variant="text" >
            <CloseIcon color="#000033" />
          </IconButton>
        </>
      ) : (
        <>
          <InnerContainer style= {{backgroundColor: '#3399ff'}}>
            <picture>
          ...
          <IconButton onClick={hideBanner} variant="text" style= {{backgroundColor: '#3399ff'}}>
            <CloseIcon color="#000033" />
          </IconButton>
        </>
      )}
