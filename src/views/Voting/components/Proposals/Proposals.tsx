...
const Proposals = () => {
  ...
  return (
    <Container py="40px" style={{background:"linear-gradient(139.73deg,#000033 0%,#008080 60%,#000033 100%)"}} >
      <Box mb="48px">
        <Breadcrumbs>
          <Link to="/">{t('Home')}</Link>
          <Text>{t('Voting')}</Text>
      ...
      <Heading as="h2" scale="xl" mb="32px" color='white' id="voting-proposals" >
        {t('Proposals')}
      </Heading>
      <Card style={{background:"#090909"}} >
        ...
        {isIdle && filteredProposals.length === 0 && (
          <Flex style={{background:"linear-gradient(139.73deg,#ccffff 0%,#009999 60%, #009999 100%)"}} alignItems="center" color='black' justifyContent="center" p="32px">
            <Heading as="h5">{t('Proposals not found')}</Heading>
          </Flex>
        )}
      </Card>
    </Container>
