  ...
  return (
    <>
      <GridCell isTopPosition={isTopPosition} pl={['12px', '24px']}>
        <Flex>
          <Text bold={isTopPosition} color={samePositionAsAbove ? 'textDisabled' : '#cc0000'} textTransform="uppercase">
            #{position}
          </Text>
        </Flex>
      </GridCell>
      ...
      <GridCell isTopPosition={isTopPosition}>
        <Flex flexDirection="column" width="100%" justifyContent="flex-end" pr={[null, null, '24px']}>
          ...
          {cakePriceBusd.gt(0) ? (
            <Text fontSize="12px" color="#0099ff" textAlign="right">
      ...
      <GridCell isTopPosition={isTopPosition}>
        <SubMenu component={<EllipsisIcon height="16px" width="16px" />}>
          {projectSite && (
            <SubMenuItem as={LinkExternal} href={projectSite} bold={false} color="#0099ff">
              {t('Project Site')}
            </SubMenuItem>
          )}
          {lpAddress && (
            <SubMenuItem as={LinkExternal} href={`/info/pool/${lpAddress}`} bold={false} color="#0099ff">
              {t('LP Info')}
            </SubMenuItem>
          )}
          {account && (
            <SubMenuItem as={LinkExternal} href={getBscScanLink(account, 'address')} bold={false} color="black">
              {t('Bidder Address')}
            </SubMenuItem>
          )}
        </SubMenu>
  ...
  return (
    <Box>
      <LeaderboardContainer>
        <Text color="#009999" bold fontSize="12px" textTransform="uppercase" pl={['12px', '24px']} py="16px">
          {t('Position')}
        </Text>
        <Text color="#009999" bold fontSize="12px" textTransform="uppercase" pl={['8px']} py="16px">
          {t('Farm')}
        </Text>
        <Text
          color="#009999"
      ...
      <Flex mt="16px" px="24px" flexDirection="column" justifyContent="center" alignItems="center">
        {visibleBidders <= 10 && totalBidders > 10 && (
          <Text color="#0099ff" textAlign="center">
