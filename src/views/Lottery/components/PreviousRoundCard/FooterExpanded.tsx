...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
    ...
    return (
      <>
        {prizeInBusd.isNaN() ? (
          ...
          <Heading scale="xl" lineHeight="1" color="#009999">
  ...
  return (
    <NextDrawWrapper style={{background:'#ffff99'}}>
      <Flex mr="24px" flexDirection="column" justifyContent="space-between">
        <Box>
          <Heading color='#009900'>{t('Prize pot')}</Heading>
          {getPrizeBalances() }
        </Box>
        <Box mb="24px">
          <Flex>
            <Text fontSize="14px" display="inline" color='#009900'>
