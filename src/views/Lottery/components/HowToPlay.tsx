...
const StepCard: React.FC<{ step: Step } > = ({ step }) => {
  return (
    <StyledStepCard width="100%" style={{background:'linear-gradient(139.73deg,#000033 0%,#008080 60%,#000033 100%)'}}>
      <StepCardInner height={['200px', '180px', null, '200px']}>
        <Text mb="16px" color='#003366' fontSize="12px" bold textAlign="right" textTransform="uppercase">
          {step.label}
        </Text>
        <Heading mb="16px" scale="lg" color="#00aaff">
          {step.title}
        </Heading>
        <Text color="#003366">{step.subtitle}</Text>
      </StepCardInner>
    </StyledStepCard>
  )
}
    ...
    <StyledStepCard width={['280px', '330px', '330px']}>
      <StepCardInner height="210px">
        <MatchExampleContainer>
          <ExampleBalls />
          <Flex>
            <Text lineHeight="72px" textAlign="right" color="#ff0000" bold mr="20px">
              {t('A')}
            </Text>
    ...
    <StyledStepCard width={['280px', '330px', '380px']}>
      <StepCardInner height="auto">
        ...
        <Flex justifyContent="space-between">
          <Text fontSize="12px" color="#660033" bold textTransform="uppercase">
            {t('Digits matched')}
          </Text>
          <Text fontSize="12px" color="#660033" bold textAlign="right" textTransform="uppercase">
    ...
    <Box width="100%" >
      ...
      <GappedFlex flexDirection={['column', 'column', 'column', 'row']}>
        <Flex flex="2" flexDirection="column">
          <Heading mb="24px" scale="lg"  color="#003366" >
            {t('Winning Criteria')}
          </Heading>
          <Heading mb="24px" scale="md" color="#00aaff">
            {t('The digits on your ticket must match in the correct order to win.')}
          </Heading>
          <Text mb="16px" color="#003366">
            {t('Here’s an example lottery draw, with two tickets, A and B.')}
          </Text>
          <BulletList>
            <li>
              <Text display="inline" color="#003366">
                {t(
      ...
      <GappedFlex flexDirection={['column', 'column', 'column', 'row']}>
        <Flex flex="2" flexDirection="column">
          <Heading mb="24px" scale="lg" color="#00aaff">
            {t('Prize Funds')}
          </Heading>
          <Text color="#003366">{t('The prizes for each lottery round come from three sources:')}</Text>
          <Heading my="16px" scale="md" color="#ffcc00">
            {t('Ticket Purchases')}
          </Heading>
          <BulletList>
            <li>
          ...
          <BulletList>
            <li>
              <Text display="inline" color="#003366">
                ...
                <InlineLink href="https://docs.pancakeswap.finance/tokenomics/cake/cake-tokenomics" color="#ffcc00">
                  {t('CAKE Tokenomics')}
                </InlineLink>
              </Text>
            </li>
      ...
      <Flex justifyContent="center" alignItems="center" flexDirection={['column', 'column', 'row']}>
        ...
        <Flex maxWidth="300px" flexDirection="column">
          <Heading mb="16px" scale="md" color="#cc0000">
            {t('Still got questions?')}
          </Heading>
          <Text color="#003366">
            {t('Check our in-depth guide on')}{' '}
            <InlineLink href="https://docs.pancakeswap.finance/products/lottery/lottery-guide" color="#ffcc00">
