    ...
    >
      <ScrollableContainer>
        <Flex justifyContent="space-between" mb="16px">
          <Text color="#0099ff">{t('Total cost')}:</Text>
          <Text>~{totalCost} CAKE</Text>
        </Flex>
        <Text fontSize="12px" color="#0099ff" mb="16px">
          {t(
            'Numbers are randomized, with no duplicates among your tickets. Tap a number to edit it. Available digits: 0-9',
          )}
        </Text>
        <Button disabled={isConfirming} mb="16px" variant="primary"  width="100%" height="32px" onClick={randomize}>
          {t('Randomize')}
        </Button>
        {tickets.map((ticket) => (
          <TicketInput
