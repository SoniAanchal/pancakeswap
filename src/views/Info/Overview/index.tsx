...
const Overview: React.FC = () => {
  ...
  return (
    <Page>
      <Heading scale="lg" mb="16px" id="info-overview-title" color="#1FC7D4"> 
        {t('PancakeSwap Info & Analytics')}
      </Heading>
      <ChartCardsContainer>
        <Card>
          <Box p={['16px', '16px', '24px']} style={{background:'linear-gradient(139.73deg, rgb(255, 230, 238) 0%, rgb(153, 204, 255) 60%, rgb(255, 230, 238) 100%)'}}>
            <Text bold color="#669999">
              {t('Liquidity')}
            </Text>
            {liquidityHover > 0 ? (
              <Text bold fontSize="24px" color="#ffcc00">
                ${formatAmount(liquidityHover)}
              </Text>
            ) : (
              <Skeleton width="128px" height="36px" />
            )}
            <Text color='#669999'>{liquidityDateHover ?? currentDate}</Text>
            <Box height="250px">
              <LineChart
        ...
        <Card>
          <Box p={['16px', '16px', '24px']} style={{background:' #4d0019'}}>
            <Text bold color="#669999">
              {t('Volume 24H')}
            </Text>
            {volumeHover > 0 ? (
            ...
            <Text color='#669999'>{volumeDateHover ?? currentDate}</Text>
            <Box height="250px"  color='#ffcc00'  >
              <BarChart data={formattedVolumeData} setHoverValue={setVolumeHover} setHoverDate={setVolumeDateHover} />
            </Box>
          </Box>
        </Card>
      </ChartCardsContainer>
      <Heading scale="lg" mt="40px" mb="16px" color='#660033'>
        {t('Top Tokens')}
      </Heading>
