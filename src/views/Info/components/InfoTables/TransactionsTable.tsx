...
const DataRow: React.FC<{ transaction: Transaction }> = ({ transaction }) => {
  ...
  return (
    <ResponsiveGrid style={{background:'#e0ebeb'}}>
      ...
      <Text color='#ffcc00'> ${formatAmount(transaction.amountUSD)}</Text>
      <Text>
        <Text color='#002633' >{`${formatAmount(abs0)} ${transaction.token0Symbol}`}</Text>
      </Text>
      <Text>
        <Text color='#002633'>{`${formatAmount(abs1)} ${transaction.token1Symbol}`}</Text>
      </Text>
      <LinkExternal color='#002633' href={getBscScanLink(transaction.sender, 'address')}>
        {truncateHash(transaction.sender)}
      </LinkExternal>
      <Text color='#002633'>{formatDistanceToNowStrict(parseInt(transaction.timestamp, 10) * 1000)}</Text>
    </ResponsiveGrid>
  )
}
      ...
      <Flex mb="16px" > 
        ...
        <Flex flexDirection={['column', 'row']}>
          <RadioGroup onClick={() => handleFilter(TransactionType.MINT)}>
            ...
            <Text ml="8px" color='#009999'>{t('Adds')}</Text>
          </RadioGroup>

          <RadioGroup onClick={() => handleFilter(TransactionType.BURN)}>
            <Radio onChange={() => null} scale="sm" checked={txFilter === TransactionType.BURN} />
            <Text ml="8px" color='#009999'>{t('Removes')}</Text>
          </RadioGroup>
        </Flex>
      </Flex>
      <TableWrapper>
        <ResponsiveGrid>
          <Text color="#008000" fontSize="12px" bold textTransform="uppercase">
            {t('Action')}
          </Text>
        ...
        {transactions ? (
          <>
            ...
            <PageButtons>
              ...
              >
                <ArrowBackIcon color={page === 1 ? ' #ffcc00' : '#990033'} />
              </Arrow>

              <Text color='#990033'>{t('Page %page% of %maxPage%', { page, maxPage })}</Text>
              <Arrow
                onClick={() => {
                  setPage(page === maxPage ? page : page + 1)
                }}
              >
                <ArrowForwardIcon color={page === maxPage ? ' #ffcc00' : '#990033'} />
              </Arrow>
            </PageButtons>
          </>
        ) : (
