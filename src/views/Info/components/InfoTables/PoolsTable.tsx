    ...
    <LinkWrapper to={`/info/pool/${poolData.address}`} >
      <ResponsiveGrid style={{background:'#e0ebeb'}}>
        ...
        <Flex>
          ...
          <Text ml="8px" color="#660033">
            {poolData.token0.symbol}/{poolData.token1.symbol}
          </Text>
        </Flex>
        <Text color="#ffcc00">${formatAmount(poolData.volumeUSD)}</Text>
        <Text color="#ffcc00">${formatAmount(poolData.volumeUSDWeek)}</Text>
        <Text color="#ffcc00">${formatAmount(poolData.lpFees24h)}</Text>
        <Text color="#ffcc00">{formatAmount(poolData.lpApr7d)}%</Text>
        <Text color="#ffcc00">${formatAmount(poolData.liquidityUSD)}</Text>
      </ResponsiveGrid>
    </LinkWrapper>
  )
}
...
const PoolTable: React.FC<PoolTableProps> = ({ poolDatas, loading }) => {
  ...
  return (
    <TableWrapper >
      <ResponsiveGrid>
        <Text color="#008000" fontSize="12px" bold>
          #
        </Text>
      ...
      {sortedPools.length > 0 ? (
        <>
          ...
          <PageButtons >
            ...
            <Text color="#660033">{t('Page %page% of %maxPage%', { page, maxPage })}</Text>
            ...
            >
              <ArrowForwardIcon color={page === maxPage ? '#660033' : '#660033'} />
            </Arrow>
          </PageButtons>
        </>
      ) : (
