...
const OptionButton = styled.div<{ enabled: boolean }>`
  ...
  color: ${({ theme, enabled }) => (enabled ? theme.card.background : theme.colors.primary)};
  :hover {
    opacity: 0.6;
    cursor: pointer;
  }
    ...
    return (
      <>
        ...
        {!showWatchlist && debouncedSearchTerm.length < MINIMUM_SEARCH_CHARACTERS && (
          <Text color= '#004d66'>{t('Search pools or tokens')}</Text>
        )}
      </>
    )
  }
  ...
  return (
    <>
      ...
      <Container >
        ...
        />
        <Menu hide={!showMenu} ref={menuRef} style={{background:'#f2f2f2'}}>
          <Flex mb="16px"  >
          ...
          {error && <Text color="#ff0000">{t('Error occurred, please try again')}</Text>}

          <ResponsiveGrid>
            <Text bold color="#660033">
              {t('Tokens')}
            </Text>
            {!isXs && !isSm && (
              <Text textAlign="end" fontSize="12px" color='#660033'>
                {t('Price')}
              </Text>
            )}
            {!isXs && !isSm && (
