...
const CardUserInfo: React.FC<YourScoreProps> = ({
  ...
  return (
    <Flex flexDirection="column" alignItems="center"  mt="16px" >
      <Heading scale="lg" color="#00394d" textAlign="center">
        {headingText}
      </Heading>
      <Text textAlign="center" fontSize="14px" color="#00394d" mt="4px">
        {subHeadingText}
      </Text>
      {shouldShowUserRanks && (
        <>
          {profile.nft && volume > 0 && (
            <Button mt="12px" variant="primary"  scale="sm" onClick={onPresentShareModal}>
              {t('Share Score')}
            </Button>
          )}
          <RanksWrapper>
