...
export const PricePairLabel: React.FC = () => {
  ...
  return (
    <Box pl="24px" position="relative" display="inline-block">
      ...
      <Label dir="left">
        <Title bold textTransform="uppercase" color='#007aff'>
          BNBUSDT
        </Title>
        <Price fontSize="12px" color='#ffcc00'>{`$${countUp}`}</Price>
      </Label>
    </Box>
  )
}
...
export const TimerLabel: React.FC<TimerLabelProps> = ({ interval, unit }) => {
  ...
  return (
    <Box pr="24px" position="relative">
      <Label dir="right" >
        {secondsRemaining !== 0 ? (
          <Title bold color="#007aff">
            {countdown}
          </Title>
        ) : (
          <ClosingTitle bold color="#007aff">
            {t('Closing')}
          </ClosingTitle>
        )}
        <Interval fontSize="12px" color="#007aff">{`${interval}${t(unit)}`}</Interval>
      </Label>
      <Token right={0}>
        <PocketWatchIcon />
      </Token>
