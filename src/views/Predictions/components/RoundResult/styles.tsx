...
export const PrizePoolRow: React.FC<PrizePoolRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      <Text bold color='#009999'>{t('Prize Pool')}:</Text>
      <Text bold color='#ffcc00'>{`${getPrizePoolAmount(totalAmount)} BNB`}</Text>
    </Row>
  )
}
...
export const LockPriceRow: React.FC<LockPriceRowProps> = ({ lockPrice, ...props }) => {
  ...
  return (
    <Row {...props}>
      <Text fontSize="14px" color='#ff9900'>{t('Locked Price')}:</Text>
...
const getBackgroundColor = ({
  ...
  if (isNext) {
    return 'linear-gradient(180deg, #009999 0%, #009999 100%)'
  }
