...
const SoonRoundCard: React.FC<SoonRoundCardProps> = ({ round }) => {
  ...
  return (
    <Card borderBackground={getBorderBackground(theme, 'soon')}>
      <CardHeader status="soon" icon={<WaitIcon mr="4px" width="21px" color='#007aff' />} title={t('Later') } epoch={round.epoch}  />
      <CardBody p="16px">
        <MultiplierArrow isDisabled />
        <RoundResultBox>
          <Text textAlign="center" >
            <Text bold color='#009999'>{t('Entry starts')}</Text>
            <Text fontSize="24px" bold color='#009999'>
              {`~${countdown}`}
            </Text>
          </Text>
        </RoundResultBox>
