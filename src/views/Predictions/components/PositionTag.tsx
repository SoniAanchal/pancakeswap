...
export const Tag: React.FC<TagProps> = ({ bg = '#00b300', startIcon, children, onClick, ...props }) => {
...
const PositionTag: React.FC<PositionTagProps> = ({ betPosition, children, ...props }) => {
  ...
  if (betPosition === BetPosition.HOUSE) {
    return (
      <>
        ...
        <TooltipText ref={targetRef} color="#009999" fontWeight="300" textTransform="uppercase">
          <Flex alignItems="center">
            {t('To Burn')}
            <InfoIcon width="16px" ml="4px" color="#009999" />
          </Flex>
        </TooltipText>
      </>
    )
  ...
  return (
    <Tag bg={isUpPosition ? '#00b300' : '#cc0000'} startIcon={icon} {...props}>
      {children}
    </Tag>
  )
}
