...
const PnlTab: React.FC<PnlTabProps> = ({ hasBetHistory, bets }) => {
  ...
  return hasBetHistory ? (
    <Box p="16px">
      <Text bold fontSize="24px" color="#009999" pb="24px">
        {t('Your history')}
      </Text>
      <Flex>
        <PnlChart lost={summary.lost.rounds} won={summary.won.rounds} />
        <Flex flexDirection="column" justifyContent="center" pl="24px">
          <Text bold color="#0099ff">
            {t('Net results')}
          </Text>
          <Text bold fontSize="24px" lineHeight="1" color={netResultIsPositive ? '#00b300' : '#cc0000'}>
            {`${netResultIsPositive ? '+' : ''}${formatBnb(netResultAmount)} BNB`}
          </Text>
          <Text small color="#0099ff">
            {`~$${netResultInUsd.toFixed(2)}`}
          </Text>
        </Flex>
      </Flex>
      <Box pl="8px">
        <Text mt="24px" bold color="#0099ff">
          {t('Average return / round')}
        </Text>
        <Text bold color={avgBnbWonIsPositive ? '#00b300' : '#cc0000'}>
        ...
        {hasBestRound && (
          <>
            ...
            <Flex alignItems="flex-end">
              <Text bold color="#00b300">{`+${formatBnb(summary.won.bestRound.payout)} BNB`}</Text>
              <Text ml="4px" small color="#0099ff">
