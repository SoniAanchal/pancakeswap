...
export const CostLabel: React.FC<CostLabelProps> = ({ cost, bnbBusdPrice, ...props }) => {
  ...
  return (
    <Flex alignItems="center" {...props}>
      {priceInUsd > 0 && (
        <Text fontSize="12px" color="#0099ff">{`($${priceInUsd.toLocaleString(undefined, {
...
export const MetaRow: React.FC<MetaRowProps> = ({ title, children, ...props }) => (
  <Flex alignItems="center" justifyContent="space-between" {...props}>
    <Text fontSize="12px" color="#0099ff" maxWidth="120px" ellipsis title={title}>
      {title}
    </Text>
    <Box>{children}</Box>
  </Flex>
...
export const ProfileNftTag: React.FC<NftTagProps> = (props) => {
  ...
  return (
    <NftTag icon={<CameraIcon mr="4px" width="16px" color="#0099ff" />} color="#0099ff" {...props}>
      {t('Profile')}
    </NftTag>
  )
}
...
export const WalletNftTag: React.FC<NftTagProps> = (props) => {
  ...
  return (
    <NftTag icon={<WalletFilledIcon mr="4px" width="16px" color="#009999" />} color="#009999" {...props}>
      {t('Wallet')}
    </NftTag>
  )
}
