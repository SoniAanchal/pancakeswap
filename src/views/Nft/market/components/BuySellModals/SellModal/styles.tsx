...
export const StyledModal = styled(Modal)<{ stage: SellingStage }>`
  ...
  & h2:first-of-type {
    ${({ stage, theme }) => (stagesWithBackButton.includes(stage) ? `color: "#009999"` : null)};
  }
  & svg:first-of-type {
    ${({ stage, theme }) => (stagesWithBackButton.includes(stage) ? `fill: "#009999"` : null)};
  }
`
...
export const BnbAmountCell: React.FC<BnbAmountCellProps> = ({ bnbAmount }) => {
  ...
  return (
    <Flex alignItems="center" justifyContent="flex-end">
      ...
      <Text small color="#0099ff" textAlign="right">
