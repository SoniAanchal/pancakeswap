      ...
      {status === 'idle' ? (
        ...
        <>
          <FlexGap gap="8px" alignItems="center">
            <Heading as="h3" scale="lg" color="#009999">
              {t('Start in')}
            </Heading>
            <FlexGap gap="4px" alignItems="baseline">
              {timeUntil.days ? (
                <>
                  <Heading scale="lg" color="#009999">
                    {timeUntil.days}
                  </Heading>
                  <Text color="#009999">{t('d')}</Text>
                </>
              ) : null}
              {timeUntil.days || timeUntil.hours ? (
                <>
                  <Heading color="#009999" scale="lg">
                    {timeUntil.hours}
                  </Heading>
                  <Text color="#009999">{t('h')}</Text>
                </>
              ) : null}
              <>
                <Heading color="#009999" scale="lg">
