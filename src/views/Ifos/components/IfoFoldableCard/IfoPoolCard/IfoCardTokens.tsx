...
const Label = (props) => <Text bold fontSize="12px" color="#009999" textTransform="uppercase" {...props} />
  ...
  return (
    <TokenSection primaryToken={token}>
      <Flex flexDirection="column">
        <Label color="#3399ff">{t('On sale').toUpperCase()}</Label>
        <Value color="#ffcc00">{saleAmount}</Value>
        <Text fontSize="14px" color="#3399ff">
    ...
    if (publicIfoData.status === 'coming_soon') {
      return (
        <>
          ...
          <Text fontSize="14px" color="#0099ff" pl="48px">
            {t('%ratio%% of total sale', { ratio: distributionRatio })}
          </Text>
          {message}
